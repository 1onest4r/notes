### Must be able run smoothly at 60fps
- and must have low poly 

- [ ] tree (got to be low poly at least 30 trees)
- [x] bench (inside and outside the building) (iron, wood)
- [ ] door (optional) (wood, iron)
- [x] trash
- [ ] struggle marks (blood, stain, rubbish)
- [ ] utils (poster, clock, material)
- [x] chairs
- [x] table
- [ ] floor (rock, wood)
- [ ] toilet (sink, and its stuffs)
- [ ] mirror but full of grease
- [ ] fire extinguisher set
- [ ] electrical outlet
- [ ] arm  support 
- [ ] cameras
- [ ] water dispenser
- [ ] tv's
- [ ] iron gates
- [ ] fences
- [ ] parking lot
- [ ] cars
- [ ] light poles
- [ ] shelves with big containers
- [ ] lockers
- [ ] bushes
- [ ] air suckling thing for outside
- [ ] trash bins
- [ ] warning signs such as (camera is watching keep out etc)
- [ ] directional arrows for guiding for floor 
- [ ] kiosk 

- [ ] try making vhs effect

### make it from scratch 
and i did it